...
def get_bookmark_for_id():
    return {
        get_user_input('ID of note')
    }
...
def loop():
    ...
    options = OrderedDict({
        'A': Option(
            'Add a bookmark',
            basic_commands.AddBookmarkCommand(),
            prep_call=get_new_bookmark_data,
            success_message='Bookmark added!',  # <6>
        ),
        'B': Option(
            'List bookmarks by date',
            basic_commands.ListBookmarksCommand(),  # <7>
        ),
        'F': Option(
            'Get bookmark for id',
            basic_commands.GetBookmarkCommand(),
            prep_call=get_bookmark_for_id
        ),
        'T': Option(
            'List bookmarks by title',
            basic_commands.ListBookmarksCommand(order_by='title'),
        ),
        'E': Option(
            'Edit a bookmark',
            basic_commands.EditBookmarkCommand(),
            prep_call=get_new_bookmark_info,
            success_message='Bookmark updated!'
        ),
        'D': Option(
            'Delete a bookmark',
            basic_commands.DeleteBookmarkCommand(),
            prep_call=get_bookmark_id_for_deletion,
            success_message='Bookmark deleted!',
        ),
        'G': Option(
            'Import GitHub stars',
            basic_commands.ImportGitHubStarsCommand(),
